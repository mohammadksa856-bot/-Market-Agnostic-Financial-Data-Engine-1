...
export_timeout="${FINENGINE_SUPABASE_EXPORT_TIMEOUT_SECONDS:-1800}"
...
while true; do
    write_status "running" "production snapshot export in progress"
    if timeout "$export_timeout" finengine --db "$database" export-supabase --all \
        --registry "$registry" --prune; then
        write_status "ready" "latest production snapshot exported"
    else
        write_status "failed" "Supabase export failed or timed out; see container logs"
