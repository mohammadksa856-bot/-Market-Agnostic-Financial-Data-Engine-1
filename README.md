...
The bundled portable snapshot is rebuilt from 61 reviewed manifests and currently contains:

- 21 enabled companies in the bundled snapshot: Saudi Aramco, SABIC, ACWA Power,
  stc, Tawuniya, Saudi Cement, Southern Province Cement, Yamama Cement, all ten
  listed Saudi banks, Apple, Microsoft, and NVIDIA.
...
- 4,988 current facts and 5,138 total fact versions in the clean rebuilt snapshot.
...
- 61 published source documents and 26 independently hashed raw archive entries in the
...
- Database schema version 17, catalog version 11, and 140 unit/integration/release tests (one dependency-availability test is skipped when the browser extra is installed).
...
The telecom acceptance uses STC's official H1 2026 Saudi Exchange attachment. Its numbered summary-table headings, slightly offset label/value baselines, and trailing comparison percentages are parsed deterministically. The 521,359-byte PDF produced 15 sourced facts and 23 sourced/calculated production points with zero exceptions. Parent equity remains distinct from undisclosed total equity rather than being guessed.
