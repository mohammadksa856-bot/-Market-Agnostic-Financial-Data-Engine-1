...
The bundled portable snapshot is rebuilt from 33 reviewed manifests and currently contains:

- 4 enabled companies: Saudi Aramco, Apple, Microsoft, and NVIDIA.
- 1,153 current facts and 1,243 total fact versions.
- 927 current Aramco data points and 1,017 total versions, plus 37 profile attributes, four ownership positions, 14 disclosures, eight corporate actions, 23 official daily market-price rows, and 15 point-in-time valuation metrics. Aramco 2025 alone contains 420 current rows across 298 distinct metrics. Coverage includes detailed financial, segment, operational, ESG, commercial, commitment, tax, credit-risk, lease, geographic revenue, PPE movements, and annual history for 2019–2025, plus discrete Q1/H1 2026 semantics. The 2025 production table is stored at reported precision and drives a deterministic 52.54-year reserve-life calculation with full formula lineage.
- 195 Apple facts, plus audited FY 2026 baselines for Microsoft and NVIDIA.
- 33 published source documents, eight independently hashed raw artifacts (seven issuer PDFs and one Saudi Exchange price snapshot), four persistent monitoring schedules, zero open publication exceptions, and zero dead jobs.
...
- Aramco currently populates 371 of 507 applicable catalog fields (73.2% raw coverage). A second evidence-aware score is 80.1%: it excludes 44 fields verified as non-disclosed, event-driven with no observed event, qualitative-only, or not applicable, leaving 92 genuine actionable gaps. The commercial pipeline keeps the SAR 5.358 billion advance tied to a long-term sales agreement as a numeric fact, while a 20-year Jafurah minimum-volume arrangement is retained as qualitative evidence because no committed volume was disclosed. Note 33 contingencies are archived as qualitative evidence because the report does not provide one aggregate amount. Financial notes are now 54/66, oil-and-gas operations 31/46, investor analytics 13/30, valuation 15/17, per-share 6/6, and liquidity/solvency 13/14.
- All 630 directly sourced Aramco facts resolve to an extraction row and archived official artifact. Read-only fact responses expose source URL/key, report page/table, extraction label/value, mapping confidence/method, archive path and SHA-256. Calculated facts expose their deterministic formula and dependencies.
