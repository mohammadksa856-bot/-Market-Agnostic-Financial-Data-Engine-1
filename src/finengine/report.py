...
def export_readable_report(db_path: str, html_path: str, csv_path: str) -> None:
    ...
    rows = conn.execute(
        ...
        d.dimensions_json,d.source_key,d.source_url,d.is_calculated,d.calculation,
        (SELECT e.page FROM extracted_facts e JOIN mapped_facts mm ON mm.extracted_fact_id=e.id
         JOIN normalized_facts n ON n.mapped_fact_id=mm.id
         WHERE e.source_key=d.source_key AND e.company_id=d.company_id AND mm.canonical_metric=d.metric_key
         AND e.period_end=d.period_end AND e.period_kind=d.period_kind AND e.fiscal_year=d.fiscal_year
         AND COALESCE(e.fiscal_quarter,0)=d.fiscal_quarter AND e.scope=d.scope
         AND e.dimensions_json=d.dimensions_json AND n.normalized_value=d.value_decimal
         ORDER BY e.id DESC LIMIT 1) AS source_page,
        (SELECT e.table_ref FROM extracted_facts e JOIN mapped_facts mm ON mm.extracted_fact_id=e.id
         JOIN normalized_facts n ON n.mapped_fact_id=mm.id
         WHERE e.source_key=d.source_key AND e.company_id=d.company_id AND mm.canonical_metric=d.metric_key
         AND e.period_end=d.period_end AND e.period_kind=d.period_kind AND e.fiscal_year=d.fiscal_year
         AND COALESCE(e.fiscal_quarter,0)=d.fiscal_quarter AND e.scope=d.scope
         AND e.dimensions_json=d.dimensions_json AND n.normalized_value=d.value_decimal
         ORDER BY e.id DESC LIMIT 1) AS source_table,
        (SELECT a.local_path FROM source_artifact_links l JOIN source_artifacts a USING(artifact_key)
         WHERE l.source_key=d.source_key ORDER BY a.archived_at DESC LIMIT 1) AS archived_path,
        (SELECT a.content_hash FROM source_artifact_links l JOIN source_artifacts a USING(artifact_key)
         WHERE l.source_key=d.source_key ORDER BY a.archived_at DESC LIMIT 1) AS artifact_sha256,
        m.category
    ...
    commercial_backlog = conn.execute(
        """SELECT c.name,c.symbol,b.status,b.description,b.source_url,b.payload_json
        FROM backlog_items b JOIN companies c USING(company_id)
        WHERE b.domain='commercial_pipeline' AND b.status IN ('open','ready','in_progress','blocked')
        ORDER BY c.market,c.symbol,b.priority"""
    ).fetchall()
    conn.close()
    ...
    for company, company_rows in groups.items():
        ...
        for row in company_rows:
            ...
            if row["is_calculated"]:
                evidence = f'<span class="derived">محسوب</span><small>{html.escape(row["calculation"] or "معادلة حتمية")}</small>'
            else:
                loc = " · ".join(part for part in [f'صفحة {row["source_page"]}' if row["source_page"] else "", row["source_table"] or ""] if part)
                archive = f'<small title="SHA-256: {html.escape(row["artifact_sha256"] or "")}">مؤرشف: {html.escape(row["archived_path"] or "غير متاح")}</small>'
                evidence = f'<a target="_blank" rel="noopener" href="{html.escape(row["source_url"],quote=True)}">فتح المصدر الرسمي</a><small>{html.escape(loc or "موضع المصدر غير محدد")}</small>{archive}'
            body.append(f'<tr data-company="{html.escape(row["symbol"])}" data-kind="{html.escape(row["period_kind"])}" data-category="{html.escape(row["category"])}" data-search="{html.escape(search_text)}"><td>{html.escape(row["period_end"])}</td><td>{html.escape(period)}</td><td>{html.escape(row["category"])}</td><td>{html.escape(AR_METRICS.get(row["metric"], row["metric"]))}<small>{html.escape(row["metric"])}</small></td><td>{html.escape(dimension_text)}</td><td class="value">{html.escape(shown)}</td><td>{evidence}</td></tr>')
