...
def export_readable_report(db_path: str, html_path: str, csv_path: str) -> None:
    ...
    with csv_target.open("w", encoding="utf-8-sig", newline="") as stream:
        ...
        writer.writerow(["الشركة", "السوق", "الرمز", "السنة", "الربع", "نهاية الفترة", "نوع الفترة", "التصنيف", "المقياس", "القيمة", "العملة", "الوحدة", "المصدر"])
        for row in rows:
            writer.writerow([row["name"], row["market"], row["symbol"], row["fiscal_year"], row["fiscal_quarter"] or "", row["period_end"], row["period_kind"], row["category"], AR_METRICS.get(row["metric"], row["metric"]), row["value"], row["currency"], row["unit"], row["source_url"]])
    ...
    for company, company_rows in groups.items():
        ...
        for row in company_rows:
            ...
            search_text=f'{AR_METRICS.get(row["metric"], row["metric"])} {row["metric"]}'.lower()
            body.append(f'<tr data-company="{html.escape(row["symbol"])}" data-kind="{html.escape(row["period_kind"])}" data-category="{html.escape(row["category"])}" data-search="{html.escape(search_text)}"><td>{html.escape(row["period_end"])}</td><td>{html.escape(period)}</td><td>{html.escape(row["category"])}</td><td>{html.escape(AR_METRICS.get(row["metric"], row["metric"]))}<small>{html.escape(row["metric"])}</small></td><td class="value">{html.escape(shown)}</td><td><a target="_blank" rel="noopener" href="{html.escape(row["source_url"],quote=True)}">فتح المصدر</a></td></tr>')
        sections.append(f'<section class="company-section" data-company-section="{html.escape(symbol)}"><h2>{html.escape(company)}</h2><div class="table"><table><thead><tr><th>الفترة</th><th>النوع</th><th>التصنيف</th><th>البيان</th><th>القيمة</th><th>التوثيق</th></tr></thead><tbody>{"".join(body)}</tbody></table></div></section>')
