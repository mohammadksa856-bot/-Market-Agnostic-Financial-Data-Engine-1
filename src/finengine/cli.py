...
def main():
    ...
    tadawulfs=sub.add_parser("tadawul-fs"); tadawulfs.add_argument("market",choices=["SA"]); tadawulfs.add_argument("symbol"); tadawulfs.add_argument("--raw-dir",default="data/raw"); tadawulfs.add_argument("--year",type=int,help="target a specific annual-FS year column"); tadawulfs.add_argument("--list",action="store_true",help="only list matching annual-results announcements"); tadawulfs.add_argument("--refresh",action="store_true",help="re-page the Exchange announcement feed before searching")
    ...
    if a.cmd=="tadawul-fs":
        ...
        if a.list:
            ...
            result=fetch_annual_fs(a.market,a.symbol,raw_dir=a.raw_dir,refresh=a.refresh,year=a.year)
        print(json.dumps(result,indent=2,ensure_ascii=False)); return
    if a.cmd=="ingest":
