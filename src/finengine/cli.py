...
from .connectors import SecCompanyFactsConnector, SaudiManifestConnector
...
def main():
    ...
    ingest=sub.add_parser("ingest"); ingest.add_argument("market",choices=["SA","US"]); ingest.add_argument("symbol"); ingest.add_argument("--registry",default="config/companies.json"); ingest.add_argument("--sa-manifest"); ingest.add_argument("--raw-dir",default="data/raw")
    ...
    if a.cmd=="ingest":
        ...
        if a.market=="US": connector=SecCompanyFactsConnector(os.environ.get("SEC_USER_AGENT","finengine contact@example.com"))
        else:
