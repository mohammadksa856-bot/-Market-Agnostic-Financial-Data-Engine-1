...
class StorageAndJobsTests(unittest.TestCase):
    ...
    def test_bank_gets_bank_health_not_f_score(self):
        ...
        for year in (2024, 2025):
            facts += [
                row("net_interest_income", "29000000000", year, PeriodKind.FY),
                row("total_operating_income", "39000000000", year, PeriodKind.FY),
                row("total_operating_expenses", "-11000000000", year, PeriodKind.FY),
                row("credit_impairment_charge", "-2300000000", year, PeriodKind.FY),
                row("net_income", "24000000000", year, PeriodKind.FY),
                row("operating_cash_flow", "-22000000000", year, PeriodKind.FY),
                row("loans_and_advances", "750000000000", year, PeriodKind.INSTANT),
                row("customer_deposits", "667000000000", year, PeriodKind.INSTANT),
                row("non_performing_financing", "8000000000", year, PeriodKind.INSTANT),
                row("gross_financing", "760000000000", year, PeriodKind.INSTANT),
                row("total_credit_allowances", "16000000000", year, PeriodKind.INSTANT),
                row("risk_weighted_assets", "600000000000", year, PeriodKind.INSTANT),
                row("total_regulatory_capital", "120000000000", year, PeriodKind.INSTANT),
