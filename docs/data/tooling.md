...
| **Audited FS** | company-profile "Financial Statements" tab (`statementType=6`) | `company_annual_fs_url(symbol)` navigates the profile portlet for `symbol`, reads the tab, and returns the **"Annual / \<year\>" cell's PDF** — the full audited consolidated FS (auditor's report + notes, filed ~March, ~1-4 MB). Verified: Jarir's 2025 column → `Resources/fsPdf/454_0_2026-03-31_…pdf` = 2.15 MB, "Independent Auditor's Report" + "Notes to the Consolidated Financial Statements". The portlet is per-company stateful so this needs `fetching.BrowserFetcher` (the `browser` extra). |
| Announcement fallback | announcement-details HTML | Scrapes the `/Resources/fsPdf/…pdf` attached to the annual-results announcement. **That one is usually the earnings-release summary** (~0.3-1 MB, no notes — SABIC Agri-Nutrients, Dallah, Mobily verified); a few large caps attach the full statements; some have none (Jarir). `fetch_annual_fs` tries the profile tab first, then this. |

**Bottom line:** `tadawul-fs SA <sym>` now archives the **audited** annual FS for essentially any listed company (profile tab), with the announcement feed as discovery + fallback. All through the official exchange — no third-party vendor.
